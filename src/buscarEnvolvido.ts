...
interface BuscarEnvolvidoParams {
  nome?: string;
  cpfCnpj?: string;
  ordem?: "asc" | "desc";
  limit?: 50 | 100;
...
async function httpGet(url: string, token: string): Promise<any> {
  ...
  if (!resp.ok) {
    if (resp.status === 401) {
      ...
      throw new Error("Envolvido não encontrado");
    }
...
function buildQueryParams(params: BuscarEnvolvidoParams): string {
  ...
  if (params.cpfCnpj) {
    queryParams.append("cpf_cnpj", params.cpfCnpj);
  }
  ...
  if (params.incluirHomonimos !== undefined) {
    queryParams.append("incluir_homonimos", params.incluirHomonimos.toString());
  }
  ...
  if (params.dataMinima) {
    ...
    params.dataMinima = formatDate(params.dataMinima);
  }

  if (params.dataMaxima) {
    queryParams.append("data_maxima", params.dataMaxima);
    params.dataMaxima = formatDate(params.dataMaxima);
  }
...
async function buscarEnvolvido(params: BuscarEnvolvidoParams): Promise<void> {
  try {
    ...
    const url = `${baseUrl}/envolvido/processos?${queryString}`;
    ...
    const resultado = {
      envolvidoEncontrado: response.envolvido_encontrado,
      processos,
...
export default buscarEnvolvido;
